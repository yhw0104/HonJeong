...
> 상태: 구현 완료(브랜치 `feat/place-public-data-master`).
> ⚠️ **갱신(2026-06-19)**: §3.2 적재 파이프라인은 구현 후 **앱 밖 외부 ETL로 분리**됨(앱 경량화 — proj4j·opencsv·`place/ingest` 제거, commit c135641). 런타임(스키마·엔티티·검색/주변/체크인)만 앱에 유지. 적재 절차는 [07-식당데이터-전략 §5](../../07-식당데이터-전략.md) 참조.
