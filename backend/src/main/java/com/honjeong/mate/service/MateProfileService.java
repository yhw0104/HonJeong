...
/**
 * 사용자 닉네임 검색과 공개 프로필(메이트 관점 관계상태 포함) 조회 비즈니스 로직.
 *
 * <p>사용처: UserController.
 */
@Service
public class MateProfileService {
    ...
    /**
     * 닉네임 부분일치로 활성 사용자 최대 20명 검색 (본인·차단 상대 제외, 메이트 여부·신청 관계상태 포함).
     *
     * @param viewerId 검색하는 사용자 ID
     * @param q 닉네임 검색어(공백이면 빈 목록 반환)
     * @return 검색 결과 목록
     */
    ...
    /**
     * 다른 사용자의 공개 프로필 조회 — 차단 관계면 404로 존재 은닉, 현재 체크인(온라인)·선호 음식·함께 먹은 횟수 합성.
     *
     * @param viewerId 조회하는 사용자 ID
     * @param targetId 조회 대상 사용자 ID
     * @return 공개 프로필(메이트 여부·신청 관계상태 포함)
     */
    @Transactional(readOnly = true)
