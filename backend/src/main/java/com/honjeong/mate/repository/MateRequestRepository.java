...
/**
 * 메이트 신청 데이터 접근 (대상 테이블: mate_requests).
 */
public interface MateRequestRepository extends JpaRepository<MateRequest, Long> {

    /**
     * 신청 단건을 발신·수신 사용자와 함께 조회 (JOIN FETCH — 권한 검사·DTO 변환 시 N+1 방지).
     *
     * @param id 신청 ID
     * @return 신청(없으면 empty)
     */
    ...
    /**
     * 내가 받은 메이트 신청 목록을 최신순 조회 (상태 필터 선택, 차단 상대의 신청 제외, 발신·수신 사용자 JOIN FETCH).
     * <p>차단 관계(양방향) 상대(fromUser=신청자)의 신청은 상호 은닉을 위해 제외한다(FR-108). excludedUserIds는 항상 non-empty.
     *
     * @param userId 수신자 ID
     * @param status 상태 필터(null이면 전체)
     * @param excludedUserIds 제외할 사용자 ID 목록
     * @return 받은 신청 목록
     */
    @Query("""
            SELECT mr FROM MateRequest mr
    ...
    /**
     * 내가 보낸 메이트 신청 목록을 최신순 조회 (상태 필터 선택, 차단 상대에게 보낸 신청 제외, 발신·수신 사용자 JOIN FETCH).
     * <p>차단 관계(양방향) 상대(toUser=수신자)로 보낸 신청은 상호 은닉을 위해 제외한다(FR-108). excludedUserIds는 항상 non-empty.
     *
     * @param userId 발신자 ID
     * @param status 상태 필터(null이면 전체)
     * @param excludedUserIds 제외할 사용자 ID 목록
    ...
    /**
     * 두 사용자 사이 특정 방향·특정 상태의 신청 단건 조회 (관계상태 판정·역방향 PENDING 정리용).
     *
     * @param fromUserId 발신자 ID
     * @param toUserId 수신자 ID
     * @param status 신청 상태
     * @return 신청(없으면 empty)
     */
    ...
    /**
     * 두 사용자 사이 from→to 방향의 PENDING 신청을 지정 상태로 일괄 종결 (차단 시 자동 정리용).
     * <p>두 유저 사이 방향별 PENDING 신청 일괄 종결(차단 자동 정리용). from→to 방향만 갱신한다.
     *
     * @param fromId 발신자 ID
     * @param toId 수신자 ID
     * @param status 종결 상태(CANCELED/DECLINED)
     * @param now 응답 시각
     * @return 갱신된 행 수
     */
    @Modifying
    ...
    /**
     * 사용자가 관련된(발신·수신 무관) 메이트 신청을 전부 삭제(탈퇴 시 관계 정리용).
     *
