...
public interface MateRequestRepository extends JpaRepository<MateRequest, Long> {
    ...
    /** 차단 관계(양방향) 상대(fromUser=신청자)의 신청은 상호 은닉을 위해 제외한다(FR-108). excludedUserIds는 항상 non-empty. */
    @Query("""
            SELECT mr FROM MateRequest mr
            JOIN FETCH mr.fromUser
            JOIN FETCH mr.toUser
    ...
    /** 차단 관계(양방향) 상대(toUser=수신자)로 보낸 신청은 상호 은닉을 위해 제외한다(FR-108). excludedUserIds는 항상 non-empty. */
    @Query("""
            SELECT mr FROM MateRequest mr
            JOIN FETCH mr.fromUser
            JOIN FETCH mr.toUser
