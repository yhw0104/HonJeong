...
/**
 * 업로드 파일 검증(빈 파일·이미지 content-type) 후 FileStorage에 저장을 위임하고 접근 URL 응답을 만든다.
 *
 * <p>사용처: FileController.
 * <p>파일 업로드 도메인 서비스. 업로드 요청을 검증한 뒤 실제 저장은 {@link FileStorage}에 위임한다.
 *
...
public class FileService {
    ...
    /**
     * 업로드 파일을 검증(빈 파일·이미지 여부)하고 저장한 뒤 접근 URL을 반환.
     * <p>업로드 파일을 검증·저장하고 접근 URL을 반환한다.
     *
