...
/**
 * 토큰 발급·관리 서비스 — JWT access 토큰과 불투명 refresh 토큰(해시 저장)의 발급·회전(rotate)·
 * 무효화(revoke)를 담당한다.
 *
 * <p>사용처: 컨트롤러가 직접 주입받지 않고 AuthService를 거쳐 쓴다.
 *
 * <p>인증에 쓰는 두 종류의 토큰을 다룬다.
 *
...
public class TokenService {
    ...
    /**
     * 불투명(opaque) refresh 토큰 원문을 생성한다.
     *
     * <p>암호학적으로 안전한 난수 32바이트를 만든 뒤, URL-safe Base64(패딩 없음)로 인코딩해 문자열로
     * 돌려준다. JWT처럼 의미를 담지 않는 순수 난수라서 추측이 불가능하다.
     *
    ...
    /**
     * 문자열을 SHA-256으로 해시해 16진수 문자열로 반환한다.
     *
     * <p>refresh 원문을 저장·조회할 때 항상 이 메서드를 거치므로, 같은 원문은 항상 같은 해시(64자리 hex)가
     * 되어 DB의 저장 해시와 대조할 수 있다.
     *
