...
/**
 * 1. 기능: 개발용 Mock SMS 발송기 — 실제 발송 대신 인증번호를 로그로만 출력(honjeong.sms.mode=mock 또는 미지정 시 활성)
 * 2. 사용처: AuthService(sendPhoneCode) — SmsSender 구현체로 주입
 *
 * <p>[기존 주석] 개발용 Mock 구현. 실제로 SMS를 보내지 않고 인증번호를 애플리케이션 로그로만 출력한다. 덕분에 개발 중에는
...
public class MockSmsSender implements SmsSender {
    ...
    /**
     * 기능: 실제 SMS 발송 대신 수신 번호와 인증번호를 로그로 출력
     * Request: phone — 수신 번호, code — 인증번호
     * Response: 없음(void)
     *
     * <p>[기존 주석] 실제 발송 대신 번호와 인증번호를 로그로 남긴다.
     *
