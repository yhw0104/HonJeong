 ...
 * (매핑 테이블: users)
 *
 * <p>온보딩 시작 시 status=PENDING으로 생성되고(휴대폰/소셜 식별만), /auth/complete에서
...
public class User extends BaseTimeEntity {
     ...
     * 상대방의 대화가 함께 사라지기 때문이다. 닉네임이 null이 되므로 표시 계층이 {@code DisplayNames}로
     * '알 수 없음'을 보여준다(리뷰·대화·같이먹기 신청 이력·알림). 공개 프로필은 차단과 같은 방식으로 404로 숨긴다.
     *
