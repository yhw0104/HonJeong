...
/**
 * 프로필 카드용 활동요약 집계 서비스(읽기 전용). 사용자 도메인이 체크인·즐겨찾기 도메인의
...
public class UserActivityService {
    ...
    public UserActivityService(CheckInRepository checkInRepository, FavoriteRepository favoriteRepository) {
        this.checkInRepository = checkInRepository;
        this.favoriteRepository = favoriteRepository;
    }

    /**
     * 사용자의 활동요약(혼밥·즐겨찾기 카운트)을 집계한다. 메이트 수는 도메인 도입 전까지 0이다.
     *
    ...
    public ActivitySummaryResponse getActivitySummary(long userId) {
        ...
        return new ActivitySummaryResponse(checkInCount, favoriteCount, mateCount);
    }
}
