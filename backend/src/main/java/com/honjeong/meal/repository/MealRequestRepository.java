...
/**
 * 같이먹기 신청 데이터 접근 — 권한검사용 단건 조회, 받은/보낸 목록, 일괄 만료(수락 정리·차단 정리·TTL 정리).
 * (대상 테이블: meal_requests)
 *
 * <p>중복 방지는 DB 유니크(uq_meal_request_from_target)가 강제하고, 여기서는 조회 쿼리를 제공한다.
...
public interface MealRequestRepository extends JpaRepository<MealRequest, Long> {
     ...
     *
     * <p>수신자에 접근할 때 추가 쿼리가 나가지 않도록 JOIN FETCH로 즉시 로딩한다.
     *
     * @param id 신청 id
    ...
    /**
     * 내가 수신자인(toCheckIn.user = me) 신청 목록을 createdAt 내림차순으로 조회한다.
     *
     * <p>status가 null이면 전체다. 신청자·수신자·장소를 전부 fetch join해 닉네임·장소명 N+1을 막는다.
    ...
    /**
     * 내가 신청자인(fromUser = me) 신청 목록을 createdAt 내림차순으로 조회한다.
     *
     * <p>status가 null이면 전체다. 신청자·수신자·장소를 전부 fetch join해 닉네임·장소명 N+1을 막는다.
    ...
    /**
     * 같은 대상 체크인으로 온 나머지 PENDING 신청을 일괄 만료한다(하나 수락 시 자동 정리).
     *
     * <p>수신자가 한 신청을 수락하면 같은 체크인으로 온 나머지 PENDING은 자리가 차서 무효가 된다. 수신자가 직접
     * 거절한 게 아니므로 DECLINED가 아니라 EXPIRED(만료)로 정리한다 — 목록에 "거절함"이 아니라 "만료됨"으로 보인다.
     *
     * @param toCheckInId 대상 체크인 ID
     * @param exceptId 방금 수락한 신청 ID(정리에서 제외)
     * @param now 응답 시각
     * @return 만료 처리된 행 수
     */
    @Modifying
    ...
    /**
     * 두 유저 사이(방향 무관)의 PENDING 신청을 일괄 만료한다 — BlockService가 차단 시 자동 정리용으로 호출한다.
     *
     * <p>차단으로 자동 정리되는 신청은 수신자가 직접 거절한 게 아니므로 EXPIRED(만료)로 처리한다
     * (차단 사실도 노출되지 않는다).
     *
     * @param a 한쪽 사용자 ID
     * @param b 다른쪽 사용자 ID
    ...
    /**
     * 대상 체크인으로 온 모든 PENDING 신청을 일괄 만료한다 — 대상이 모집(SEEKING)을 벗어날 때
     * (혼자먹기·그만두기) 자동 정리용이다.
     *
     * <p>같이먹기 신청은 대상이 SEEKING일 때만 수락 가능하다. 대상이 혼자먹기(ACTIVE)나 그만두기(CANCELLED)로
    ...
    /**
     * 더는 모집(SEEKING) 상태가 아닌 대상 체크인에 걸린 PENDING 신청을 일괄 만료한다 —
     * 만료 스케줄러의 catch-all 정리용이다.
     *
     * <p>TTL 만료로 SEEKING이 CANCELLED가 되는 등 상호작용 경로를 놓친 잔여 PENDING까지 쓸어담는 안전망이다.
    ...
    /**
     * 한 사용자가 관련된 모든 PENDING 같이먹기 신청을 EXPIRED로 종결한다(탈퇴 정리용).
     *
     * <p>수신자는 to_check_in → check_ins.user_id 로만 식별되므로 서브쿼리로 찾는다.
     * 상대가 직접 거절한 게 아니므로 DECLINED가 아니라 EXPIRED다({@link #expirePendingBetween}과 같은 의도).
     *
