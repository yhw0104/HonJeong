...
/**
 * 모든 API 응답을 감싸는 공통 응답 엔벨로프. 성공: {@code {success:true, data:...}},
 * 실패: {@code {success:false, error:{code,message}}}.
 *
 * <p>사용처: 전 도메인 Controller(성공 응답 래핑), GlobalExceptionHandler(예외→실패 응답 변환),
 * SecurityConfig(401/403 JSON 직렬화 시 동일 형식 수기 작성).
 *
 * <p>직접 생성하지 않고 정적 팩토리({@link #success}/{@link #error})로 만든다.
...
public record ApiResponse<T>(boolean success, T data, ApiError error) {
    ...
    /**
     * 성공 응답을 만든다. {@code success=true}, 본문은 {@code data}, {@code error}는 null이라
     * 직렬화에서 빠진다.
     *
    ...
    /**
     * 실패 응답을 만든다. {@code success=false}, {@code data}는 null이라 직렬화에서 빠지고
     * {@code error}에 코드·메시지를 담는다.
     *
