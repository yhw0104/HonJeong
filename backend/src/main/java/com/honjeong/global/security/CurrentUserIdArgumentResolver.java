...
/**
 * {@link CurrentUserId}가 붙은 Long 파라미터에 SecurityContext의 JWT sub(userId)를 주입하는 MVC 인자 리졸버.
 * 인증 컨텍스트가 없거나 JWT principal이 아니면 401.
 *
 * <p>사용처: WebConfig(addArgumentResolvers)가 등록해 {@code @CurrentUserId}를 쓰는 모든 컨트롤러 핸들러에
 * 적용된다.
 */
public class CurrentUserIdArgumentResolver implements HandlerMethodArgumentResolver {
     ...
     *
     * <p>{@code @CurrentUserId}가 붙어 있고 타입이 {@code Long} 또는 {@code long}일 때만 true.
     *
     ...
     *
     * <p>인증 컨텍스트가 없거나 principal이 JWT가 아니면 {@link ErrorCode#UNAUTHORIZED}(401)를 던진다.
     * MVC 리졸버 규약상 4개 인자를 받지만 실제로는 SecurityContext만 사용한다.
     *
