 ...
 * 검증기가 된다. 이 구현은 카카오·애플이 서명한 ID 토큰을 검증하지 않고 <b>받은 문자열로 신원을
 * 합성</b>한다({@code "mock-kakao-" + 받은문자열}). 배포 서버가 이 모드로 떠 있으면 누구든 임의
 * 문자열을 {@code POST /api/auth/oauth/{provider}}에 넣어 계정을 무제한 만들 수 있고, mock으로 떠
 * 있는 동안 만들어진 계정은 그 문자열을 아는 사람 누구나 그대로 로그인할 수 있다.
 *
 * <p><b>정확히 해 둘 것</b>: 이 상태로 뜬다고 해서 real 모드에서 이미 만들어진 계정을 가로챌 수
 * 있는 것은 아니다. mock이 붙이는 {@code "mock-"} 접두사 때문에 합성 식별자가 실제 카카오 {@code sub}와
 * 절대 겹치지 않기 때문이다. 다만 그 접두사는 <b>방어를 의도한 것이 아니라</b> 테스트 재현성을 위한
 * 형식일 뿐이라(그 클래스의 주석이 그렇게 말한다), 언제든 형식이 바뀌면 사라지는 성질의 보호다.
 * 여기에 기대지 않는다. 이 프로젝트에서 결과가 가장 큰 설정값 하나다.
 *
    ...
    public OAuthRealModeCheck(@Value("${honjeong.oauth.mode:mock}") String oauthMode) {
        Assert.isTrue("real".equals(oauthMode),
                "honjeong.oauth.mode=" + oauthMode + "입니다. 배포 프로파일에서는 real이어야 합니다. "
                        ...
                        + "받은 문자열로 신원을 합성합니다 — 아무나 임의 문자열로 계정을 무제한 만들 수 "
                        + "있고, 그렇게 만들어진 계정은 그 문자열을 아는 사람 누구나 그대로 로그인할 수 "
                        + "있습니다(2026-07-27에 실제로 열려 있던 상태입니다). "
                        + "OAUTH_MODE=real로 두고 KAKAO_APP_KEY를 채워주세요. "
