 ...
 *
 * <p>[기존 주석] 역지오코딩 응답 DTO. {@code GET /api/geo/reverse}에서 좌표→동네 변환 결과로 반환한다.
...
public record ReverseGeocodeResponse(String region, double regionLat, double regionLng) {
    ...
    /**
     * 기능: 서비스 계층 값 객체(ReverseGeocodeResult)를 응답 DTO로 변환한다
     * Request: r — 역지오코딩 결과 값 객체
     * Response: ReverseGeocodeResponse — 동일 값을 담은 응답 DTO
     *
     * <p>[기존 주석] 서비스 계층 결과 값 객체를 응답 DTO로 변환한다.
     *
     * @param r 역지오코딩 결과
     * @return 응답 DTO
     */
