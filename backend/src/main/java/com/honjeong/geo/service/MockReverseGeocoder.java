...
/**
 * 1. 기능: 외부 API 호출 없이 좌표 해시로 5개 후보 중 결정론적 동네를 돌려주는 개발용 Mock 역지오코더(ReverseGeocoder 구현체)
 * 2. 사용처: GeoService — honjeong.geo.mode=mock 또는 미지정(기본) 시 빈 등록, 운영 시 카카오 REST 구현으로 교체 예정
 *
 * <p>[기존 주석] 개발용 Mock 역지오코더. 외부 역지오코딩 API를 호출하지 않고, 받은 좌표만으로 <b>결정론적</b> 동네를
...
public class MockReverseGeocoder implements ReverseGeocoder {
    ...
    /**
     * 기능: 좌표를 소수 둘째자리로 반올림해 해시한 값으로 5개 후보 동네 중 하나를 결정론적으로 선택한다
     * Request: lat — 위도, lng — 경도
     * Response: ReverseGeocodeResult — 후보 동네 표시명 + 입력 좌표 그대로(mock이라 중심 스냅 없음)
     */
    @Override
