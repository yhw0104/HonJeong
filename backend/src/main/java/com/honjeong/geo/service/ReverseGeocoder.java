...
/**
 * 좌표(위도·경도)→동네(행정구역) 변환 책임을 정의하는 역지오코딩 인터페이스.
 *
 * <p>사용처: GeoService(주입) — 구현체: MockReverseGeocoder(개발), 운영 시 카카오 REST 구현으로 교체 예정.
 * <p>좌표(위도·경도)를 사람이 읽는 동네(행정구역)로 바꾸는 역지오코딩 책임을 정의하는 인터페이스.
