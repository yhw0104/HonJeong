...
- `prod` — PostgreSQL, 환경변수(`DB_URL`/`DB_USERNAME`/`DB_PASSWORD`) 주입, `ddl-auto: validate`, 외부연동은 `oauth.mode`·`push.mode`·`apple.mode`가 기본 real(`PUSH_MODE`로 전환), `sms.mode`는 real 구현체가 없어 기본 mock(`SMS_MODE`로 override 가능).
- **배포 가드** — `local`·`test`를 제외한 모든 프로파일에서 `oauth.mode`는 반드시 `real`이어야 하고(`OAuthRealModeCheck`), `oauth.mode=real`이면 `apple.mode`도 `real`이어야 한다(`AppleModeConsistencyCheck`). 어기면 기동에 실패한다. 대상 프로파일을 `prod`로 나열하지 않고 제외 목록으로 적은 이유는 `DeployedProfiles` 참고 — 새 프로파일이 검사에서 조용히 빠지지 않게 하기 위함이다.
