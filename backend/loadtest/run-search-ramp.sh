#!/usr/bin/env bash
#
# 테스트 1(검색 램프업) 실행기. k6와 함께 컨테이너 CPU·메모리와 DB 커넥션 상태를 1초 간격으로 수집한다.
#
#   ./run-search-ramp.sh                       # 0.5→6건/초 6단계 × 30초
#   RATES=1,2,4,8 DURATION_S=60 ./run-search-ramp.sh
#
...
statsfile="results/$STAMP-ramp-stats.csv"
