#!/usr/bin/env bash
#
# 채팅 목록 N+1 실행기. 대화방 수별 응답 시간을 재고 컨테이너 자원을 함께 수집한다.
#
#   ./run-conversations.sh        # 대화방 0/5/20/50개 × 30초, VU 1개 순차
#
# ★사전 준비: ./seed-users.sh 300 && python3 seed-conversations.py
...
[ -f conv-groups.json ] || { echo "conv-groups.json이 없다. python3 seed-conversations.py 를 먼저 돌릴 것." >&2; exit 1; }
...
statsfile="results/$STAMP-convload-stats.csv"
