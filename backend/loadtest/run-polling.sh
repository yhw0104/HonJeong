#!/usr/bin/env bash
#
# 테스트 3(폴링 부하) 실행기. 컨테이너 자원·DB 커넥션에 더해 **GC 로그**를 함께 모은다.
#
#   ./run-polling.sh                           # 50→100→200→300명 × 60초
#   USERS=100,300 DURATION_S=120 ./run-polling.sh
#
# ★GC 로깅 오버레이가 필요하다(JRE 이미지에 jcmd가 없어 GC를 볼 방법이 로그뿐이다):
#   docker compose -f docker-compose.yml -f docker-compose.loadtest.yml \
#                  -f docker-compose.gclog.yml up -d db app
#
...
need="$(echo "${USERS:-50,100,200,300}" | tr ',' '\n' | sort -n | tail -1)"
[ "$ntok" -ge "$need" ] || { echo "토큰이 ${ntok}개뿐이다. 최대 ${need}명을 흉내내려면 ./seed-users.sh $need 를 먼저 돌릴 것." >&2; exit 1; }
...
docker exec honjeong-app sh -c 'cat /proc/1/environ | tr "\0" "\n" | grep -q "^JAVA_TOOL_OPTIONS=.*Xlog:gc"' 2>/dev/null || {
  echo "★ GC 로깅이 꺼져 있다. docker-compose.gclog.yml을 겹쳐 다시 띄울 것:" >&2
  echo "  docker compose -f docker-compose.yml -f docker-compose.loadtest.yml -f docker-compose.gclog.yml up -d db app" >&2
  exit 1
}
gcmark="$(docker logs honjeong-app 2>&1 | wc -l)"   # 이 지점 이후의 GC 로그만 센다
echo "토큰 ${ntok}개 · 제한 2CPU/1900MB · GC 로깅 확인 · 결과 접두사 $STAMP"
...
statsfile="results/$STAMP-polling-stats.csv"
...
STAMP="$STAMP" k6 run \
  --env "STAMP=$STAMP" \
  --env "USERS=${USERS:-50,100,200,300,50}" \
...
echo
# ── GC 요약 ─────────────────────────────────────────────────────────────────
gclog="results/$STAMP-gc.log"
docker logs honjeong-app 2>&1 | tail -n +$((gcmark+1)) | grep -E "\[gc" > "$gclog" || true
echo
echo "GC (힙 상한 476MB = 컨테이너 1900MB의 25%)"
# ★awk의 3인자 match()는 gawk 확장이라 macOS 기본 awk에서 syntax error로 죽는다.
#   실제로 1차 실행이 여기서 exit 2로 끝났다(측정은 다 끝난 뒤라 데이터는 남았다).
python3 - "$gclog" <<'PYGC'
import re, sys
lines = open(sys.argv[1]).read().splitlines()
ev = [m for m in (re.search(r'Pause .*?(\d+)M->(\d+)M\((\d+)M\) ([\d.]+)ms', l) for l in lines) if m]
if not ev:
    print("  GC 이벤트 없음 — 원본을 직접 볼 것"); raise SystemExit
ms = [float(m.group(4)) for m in ev]
def upt(l): return float(l.split('][')[1].rstrip('s]'))
span = upt(lines[-1]) - upt(lines[0]) or 1
print(f"  일시정지 {len(ev)}회  합계 {sum(ms):.0f}ms  최대 {max(ms):.1f}ms  평균 {sum(ms)/len(ms):.1f}ms")
print(f"  빈도 {len(ev)/span:.1f}회/초   STW 비율 {sum(ms)/1000/span*100:.2f}%")
print(f"  힙 committed {ev[0].group(3)}MB → {ev[-1].group(3)}MB   GC 후 최대 {max(int(m.group(2)) for m in ev)}MB")
PYGC
echo "  원본: $gclog"
echo
